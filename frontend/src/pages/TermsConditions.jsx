...
const TermsConditions = () => (
  <section className="py-20 px-6 max-w-4xl mx-auto min-h-screen">
    ...
    <div className="mb-12">
      ...
      <h1 className="text-4xl font-black text-gray-900 dark:text-zinc-50 mb-3">Terms &amp; Conditions</h1>
      ...
      <p className="text-gray-500 dark:text-zinc-400 text-base mt-2">
    ...
    <div className="space-y-5 mb-14">
      {sections.map((s, i) => (
        <div
          key={i}
          className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-sm p-6 flex gap-5"
        >
          <span className="text-3xl shrink-0 mt-0.5">{s.icon}</span>
          <div>
            <h2 className="text-base font-bold text-gray-900 dark:text-zinc-50 mb-2">{s.title}</h2>
            <p className="text-gray-600 dark:text-zinc-300 text-sm leading-relaxed">{s.body}</p>
          </div>
        </div>
      ))}
    </div>
      ...
      <ul className="divide-y divide-gray-50 dark:divide-zinc-800">
        {bookingPoints.map((p, i) => (
          <li key={i} className="flex items-start gap-3 px-6 py-4">
            ...
            <p className="text-gray-600 dark:text-zinc-300 text-sm leading-relaxed">{p}</p>
          </li>
        ))}
      </ul>
    </div>
    ...
    {/* Dispute Resolution */}
    <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-100 dark:border-amber-900/50 rounded-2xl p-6 mb-12">
      <div className="flex items-center gap-3 mb-3">
        <span className="text-2xl">⚖️</span>
        <h2 className="text-lg font-bold text-gray-900 dark:text-zinc-50">Dispute Resolution</h2>
      </div>
    ...
    {/* Contact nudge */}
    <div className="bg-gradient-to-br from-violet-50 to-indigo-50 dark:from-violet-950/30 dark:to-indigo-950/30 border border-violet-100 dark:border-violet-900/50 rounded-3xl p-8 text-center">
      <p className="text-gray-700 dark:text-zinc-300 font-semibold mb-1">Questions about our terms?</p>
      <p className="text-gray-500 dark:text-zinc-400 text-sm mb-4">Our team is happy to clarify any clause before you book.</p>
