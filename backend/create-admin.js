 ...
 *      lib/validateAdminPassword.js first, so a 16-character minimum and the
...
const { validateAdminPassword, MIN_LENGTH } = require('./lib/validateAdminPassword');
 ...
 */
async function resolvePassword(targetUsername) {
  const fromEnv = process.env.ADMIN_PASSWORD;
  if (fromEnv) {
    const check = validateAdminPassword(fromEnv, targetUsername);
    if (!check.valid) {
  ...
  if (passwordArg) {
    ...
    const check = validateAdminPassword(passwordArg, targetUsername);
    if (!check.valid) {
      console.error(`✗ ${check.error}`);
      return null;
    }
  ...
  const first = await promptHidden(`New password (min ${MIN_LENGTH} chars, not echoed): `);
  const check = validateAdminPassword(first, targetUsername);
  if (!check.valid) {
    console.error(`✗ ${check.error}`);
    return null;
  }
...
async function run() {
  try {
    ...
    switch (action) {
        ...
        if (admins.rows.length === 0) {
          ...
          admins.rows.forEach((a) => {
            ...
            console.log(
              ...
              `, sessions rev: ${a.token_version}, failed: ${a.failed_login_attempts}${locked})${pending}`
            );
          });
      ...
      case 'reset-password': {
        if (!username) {
          console.error('Usage: node create-admin.js reset-password <username>');
          process.exit(1);
        }
        const target = await pool.query('SELECT id FROM admins WHERE username = $1', [username]);
